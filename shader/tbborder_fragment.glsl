...
{
//   if(abs(viewDir.x / viewDir.y) > 2 || abs(viewDir.z / viewDir.y) > 2) heightscale = 0.1;

   vec2 xz_var = viewDir.xz / viewDir.y;

   float yheight = Position_worldspace.y;

   if(yheight > GetHeight(texCoords)) result = 1.0;
   else result = 0.0;
 
   vec2 currenttexCoords = texCoords + (maxheight- yheight) * xz_var * heightscale;
    
   const float minLayers = 50;
   const float maxLayers = 200;
   float numLayers = mix(maxLayers, minLayers, abs(dot(vec3(0, 1, 0), viewDir)));
   float layerDepth = (maxheight - yheight) * heightscale/ maxLayers;
   
   float currentLayerDepth = yheight + (maxheight - yheight) * heightscale;
   vec2 p = xz_var * (maxheight - yheight) * heightscale;
   vec2 deltaTexCoords = p / maxLayers;

   float currentDepthMapValue = GetHeight(currenttexCoords);

   int count = int(maxLayers);

   while(currentDepthMapValue < currentLayerDepth && count > 0)
   {
...
void main()
{
   ...
   vec2 uv = ParallaxMapping(Position_worldspace.xz, E);
